...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_layers: bool, net_type="resnet"):
        super().__init__()
    ...
    def forward(self, tensor_list: NestedTensor):
        xs = self.body(tensor_list.tensors)  # [16, 1024, 8, 8]
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor, mode=None):
        '''tensor_list.tensors.size() ([16, 3, 128, 128])  即传入的是一个batch的图片
           tensor_list.mask.size() ([16, 128, 128])
        '''
