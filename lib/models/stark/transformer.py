...
class Transformer(nn.Module):
    ...
    def forward(self, feat, mask, query_embed, pos_embed, mode="all", return_encoder_output=False):
        ...
        if self.encoder is None:
            ...
            memory = self.encoder(feat, src_key_padding_mask=mask, pos=pos_embed)  # encoder输出
        if mode == "encoder":
            return memory
        elif mode == "all":
            ...
            if self.decoder is not None:
                tgt = torch.zeros_like(query_embed)  # 全0tensor
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
                ...
                pos: Optional[Tensor] = None,
                query_pos: Optional[Tensor] = None):  # 意思query是调用模型时给出的query？
        output = tgt
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        if self.divide_norm:
            q = q / torch.norm(q, dim=-1, keepdim=True) * self.scale_factor
            k = k / torch.norm(k, dim=-1, keepdim=True)
        tgt2 = self.self_attn(q, k, tgt, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask)[0]  # qk是位置编码后的tgt，v是tgt
