...
class STARK_ST(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        z_patch_arr1, _, z_amask_arr1 = sample_target(image, info['init_bbox'], self.params.template_factor, output_sz=self.params.template_size)
    ...
    def track(self, image, info: dict = None):
        ...
        # 注意！！！self.state是上一帧的推理结果，这里对于search image会先以上帧的推理结果作为box，辅以factor和output_sz，切出search region，再喂入模型
        ...
        for idx, update_i in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0 and conf_score > 0.5:
                # 把当前的search region切出来作为dynamic template
                z_patch_arr, _, z_amask_arr = sample_target(image, self.state, self.params.template_factor, output_sz=self.params.template_size)  # (x1, y1, w, h)
