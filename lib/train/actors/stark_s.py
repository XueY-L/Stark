...
class STARKSActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        """
        # forward pass
        # data  torch.Size([2, 16, 3, 128, 128])
        out_dict = self.forward_pass(data, run_box_head=True, run_cls_head=False)  # [16,1,4]
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        for i in range(self.settings.num_template):
            template_img_i = data['template_images'][i].view(-1, *data['template_images'].shape[2:])  # (bs, 3, 128, 128)
            template_att_i = data['template_att'][i].view(-1, *data['template_att'].shape[2:])  # (bs, 128, 128)  # attention mask，不知道是什么
        ...
        seq_dict = merge_template_search(feat_dict_list)  # cat 特征
    ...
    def compute_losses(self, pred_dict, gt_bbox, return_status=True):
        ...
        if torch.isnan(pred_boxes).any():  # 原来是报错，现在返回None，用来跳过这个batch
            # raise ValueError("Network outputs is NAN! Stop Training")
            print("Network outputs is NAN! Stop Training")
            return None, None
